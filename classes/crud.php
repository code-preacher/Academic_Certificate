...
// Inserting
		...
		public function insertInstitution($post,$file)
	{
	...
	{
		$faculty_id=$this->cleanse($post['faculty_id']);
		$department=$this->cleanse(strtoupper($post['department']));
	...
	{
		$faculty_id=$this->cleanse($post['faculty_id']);
		$department_id=$this->cleanse(strtoupper($post['department_id']));
		$name=$this->cleanse(strtoupper($post['name']));
	...
	public function insertStudent($post)
	{
		...
		$matno=$this->cleanse(strtoupper($post['matno']));
		$password=$this->cleanse($post['password']);
		$school_id=$this->cleanse($post['school_id']);
		$faculty_id=$this->cleanse($post['faculty_id']);
...
//Inserting with Files
	...
	public function insertDataWithFiles($post,$file)
	{
		$value1 = $this->cleanse($post['value1']);
		$value2 = $this->cleanse($post['value2']);
